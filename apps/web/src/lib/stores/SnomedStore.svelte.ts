...
const snomedNameCache: Map<string, string> = createLocalStorageMap('snomedNameCache');
...
export async function getSnomedNames(codes: string[]): Promise<Map<string, string>> {
	const codesToFetch = codes.filter((code) => !snomedNameCache.has(code));
